...
class Trie {
 ...
 public:
  ...
  std::shared_ptr<TrieNode> TrieClone(std::shared_ptr<TrieNode> u) const;

  template <class T>
  std::shared_ptr<TrieNodeWithValue<T>> TrieClone(std::shared_ptr<TrieNodeWithValue<T>> u) const;
  ...
  bool dfs_remove(std::shared_ptr<TrieNode> p, std::shared_ptr<TrieNode> fa, std::string_view &key, int u) const;
